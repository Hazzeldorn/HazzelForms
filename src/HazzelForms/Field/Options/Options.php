    ...
    public function returnField()   {
        ...
        foreach($this->options as $optionKey => $option){
            ...
            $fieldHtml .= sprintf(
                ...
                $this->fieldType, $this->formName, $this->fieldSlug, $optionKey, $this->classlist, $this->buildOptionAttributeString($option)
            );
            $fieldHtml .= sprintf(
                '<label for="%1$s-%2$s-%3$s"><span class="option-caption">%4$s</span></label>',
                $this->formName, $this->fieldSlug, $optionKey, $option
            );
            $fieldHtml .= '</div>';
